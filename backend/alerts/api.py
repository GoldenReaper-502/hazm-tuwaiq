...
async def create_alert(
    ...
    try:
        ...
        matching_rules = [
            rule
            for rule in ALERT_RULES_DB.values()
            if (
                rule.organization_id == organization_id
                and rule.is_active
                and rule.trigger_type == alert.type
            )
        ]
        ...
        if matching_rules:
            ...
            notifications = notification_manager.send_alert_notification(
                alert=alert, recipients=mock_recipients, channels=rule.notify_channels
            )
...
async def list_alerts(
    ...
    try:
        ...
        return create_response(
            ...
            data={"alerts": [a.dict() for a in alerts], "total": len(alerts)},
        )
...
@router.post(
    "/alerts/{alert_id}/acknowledge", summary="تأكيد استلام التنبيه - Acknowledge Alert"
)
async def acknowledge_alert(
    ...
    try:
        success, error = alert_engine.acknowledge_alert(
            alert_id=alert_id, user_id=user_id, notes=notes
        )
...
async def resolve_alert(
    ...
    try:
        success, error = alert_engine.resolve_alert(
            alert_id=alert_id, user_id=user_id, resolution_notes=resolution_notes
        )
...
async def create_alert_rule(
    rule: AlertRule, organization_id: str = Query(..., description="Organization ID")
):
    """
    Create alert rule
...
async def get_alert_stats(
    ...
    try:
        ...
        org_alerts = [
            a for a in ALERTS_DB.values() if a.organization_id == organization_id
        ]
...
async def get_alert_timeline(
    ...
    try:
        ...
        alerts = [
            ...
            if (a.organization_id == organization_id and a.created_at >= start_date)
        ]
        ...
        for alert in alerts:
            ...
            if day not in daily_counts:
                daily_counts[day] = {"total": 0, "by_severity": {}, "by_type": {}}
