...
class StreamManager:
    ...
    def add_camera(
        ...
        with self.lock:
            ...
            try:
                # Create RTSP handler
                handler = RTSPHandler(camera_id=camera_id, rtsp_url=rtsp_url)
    ...
    def get_all_stats(self) -> Dict[str, Any]:
        ...
        # Update global stats
        total_frames = sum(
            stream.stats["frames_captured"] for stream in self.streams.values()
        )
        ...
        camera_stats = {
            camera_id: stream.get_stats() for camera_id, stream in self.streams.items()
        }

        return {"global": self.global_stats, "cameras": camera_stats}
