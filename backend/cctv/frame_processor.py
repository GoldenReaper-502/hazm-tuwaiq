...
class FrameProcessor:
    ...
    def process_frame(
        self, frame: np.ndarray, camera_id: str, full_analysis: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.enable_intent and self.intent_detector and full_analysis:
                ...
                if (
                    "detection" in results
                    and results["detection"].get("people_count", 0) > 0
                ):
    ...
    def annotate_frame(self, frame: np.ndarray, analysis: Dict[str, Any]) -> np.ndarray:
        """
