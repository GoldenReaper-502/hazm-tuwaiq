...
class PDFReportGenerator:
    ...
    def generate_safety_report(self, report_data: ReportData, output_path: str) -> str:
    ...
    def generate_compliance_report(
        self, compliance_data: ComplianceReport, output_path: str
    ) -> str:
        ...
        if compliance_data.violations:
            story.append(
                Paragraph("Non-Compliance Items", self.styles["ArabicHeading"])
            )
            ...
            violations_table = Table(
                violations_data, colWidths=[1 * inch, 3 * inch, 1 * inch, 1 * inch]
            )
    ...
    def generate_board_report(self, board_data: BoardReport, output_path: str) -> str:
        ...
        story.append(Paragraph("Executive Board Report", self.styles["ArabicTitle"]))
        ...
        story.append(
            Paragraph(board_data.executive_summary, self.styles["ExecutiveSummary"])
        )
        story.append(Spacer(1, 0.3 * inch))
        ...
        if board_data.strategic_highlights:
            story.append(
                Paragraph("Strategic Highlights", self.styles["ArabicHeading"])
            )
        ...
        story.append(
            Paragraph("Key Performance Indicators", self.styles["ArabicHeading"])
        )
        ...
        kpi_table = Table(
            [
                ...
                [
                    "Safety Score",
                    f"{board_data.safety_score:.1f}",
                    "85.0",
                    "✓" if board_data.safety_score >= 85 else "✗",
                ],
                [
                    "Incident Reduction",
                    f"{board_data.incident_reduction:.1f}%",
                    "10%",
                    "✓" if board_data.incident_reduction >= 10 else "✗",
                ],
                [
                    "Compliance Rate",
                    f"{board_data.compliance_rate:.1f}%",
                    "95%",
                    "✓" if board_data.compliance_rate >= 95 else "✗",
                ],
            ],
        ...
        financial_table = Table(
            [
                ...
                [
                    "Net Impact",
                    f"${board_data.savings_from_prevention - board_data.incident_costs:,.2f}",
                ],
            ],
            colWidths=[3 * inch, 2 * inch],
        )
        ...
        if board_data.top_risks:
            ...
            risks_table = Table(
                risks_data, colWidths=[2 * inch, 1 * inch, 1 * inch, 2.5 * inch]
            )
        ...
        if board_data.board_recommendations:
            story.append(
                Paragraph("Board Recommendations", self.styles["ArabicHeading"])
            )
    ...
    def _create_cover_page(self, report_data: ReportData) -> List:
        ...
        if report_data.description:
            elements.append(
                Paragraph(report_data.description, self.styles["ArabicBody"])
            )
        ...
        details = [
            ...
            [
                "Period",
                f"{report_data.period_start.strftime('%Y-%m-%d')} to {report_data.period_end.strftime('%Y-%m-%d')}",
            ],
    ...
    def _create_executive_summary(self, report_data: ReportData) -> List:
        ...
        elements.append(
            Paragraph(report_data.executive_summary, self.styles["ExecutiveSummary"])
        )
        elements.append(Spacer(1, 0.3 * inch))
    ...
    def _create_metrics_dashboard(self, metrics: ReportMetrics) -> List:
        ...
        metrics_data = [
            [
                Paragraph(
                    f"<b>{metrics.safety_score:.1f}</b><br/>Safety Score",
                    self.styles["ArabicBody"],
                ),
                Paragraph(
                    f"<b>{metrics.total_incidents}</b><br/>Incidents",
                    self.styles["ArabicBody"],
                ),
                Paragraph(
                    f"<b>{metrics.compliance_rate:.1f}%</b><br/>Compliance",
                    self.styles["ArabicBody"],
                ),
            ],
            [
                Paragraph(
                    f"<b>{metrics.total_near_misses}</b><br/>Near Misses",
                    self.styles["ArabicBody"],
                ),
                Paragraph(
                    f"<b>{metrics.total_violations}</b><br/>Violations",
                    self.styles["ArabicBody"],
                ),
                Paragraph(
                    f"<b>{metrics.ppe_compliance:.1f}%</b><br/>PPE Compliance",
                    self.styles["ArabicBody"],
                ),
            ],
        ]
    ...
    def _create_incidents_section(self, incidents: List[Dict[str, Any]]) -> List:
        ...
        incidents_table = Table(
            incidents_data,
            colWidths=[1.2 * inch, 1.5 * inch, 1 * inch, 1.5 * inch, 1 * inch],
        )
    ...
    def _add_header_footer(self, canvas, doc):
        ...
        canvas.drawString(
            2 * cm, 1 * cm, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
        )
