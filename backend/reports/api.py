...
async def generate_report(request: ReportRequest):
    ...
    try:
        ...
        return create_response(
            success=True, message="Report generated successfully", data=generated.dict()
        )
...
async def get_report(report_id: str):
    ...
    return create_response(
        success=True, message="Report retrieved successfully", data=report.dict()
    )
...
async def delete_report(report_id: str):
    ...
    return create_response(success=True, message="Report deleted successfully")
...
async def get_schedule(schedule_id: str):
    ...
    return create_response(
        success=True, message="Schedule retrieved successfully", data=schedule.dict()
    )
...
async def update_schedule(schedule_id: str, schedule: ScheduledReport):
    ...
    return create_response(
        success=True, message="Schedule updated successfully", data=schedule.dict()
    )
...
async def delete_schedule(schedule_id: str):
    ...
    return create_response(success=True, message="Schedule deleted successfully")
...
async def run_schedule_now(schedule_id: str):
    ...
    return create_response(
        success=True, message="Report generation triggered successfully"
    )
...
async def create_template(template: ReportTemplate):
    ...
    return create_response(
        success=True, message="Template created successfully", data=template.dict()
    )
...
async def list_templates(
    ...
    if organization_id:
        templates = [
            t for t in templates if t.organization_id == organization_id or t.is_public
        ]
...
async def get_template(template_id: str):
    ...
    return create_response(
        success=True, message="Template retrieved successfully", data=template.dict()
    )
...
async def delete_template(template_id: str):
    ...
    return create_response(success=True, message="Template deleted successfully")
...
async def generate_board_report(
    organization_id: str, start_date: datetime, end_date: datetime
):
    ...
    try:
        ...
        metrics = ReportMetrics(
            total_incidents=15, safety_score=87.5, compliance_rate=94.2
        )
        ...
        initiatives = {
            ...
            "ongoing": [{"name": "AI Safety Monitoring Deployment", "progress": "75%"}],
            "planned": [
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate board report: {str(e)}"
        )
...
async def get_report_analytics():
    ...
    analytics = ReportAnalytics(
        ...
        most_requested_type=(
            max(reports_by_type.items(), key=lambda x: x[1])[0]
            if reports_by_type
            else None
        ),
    )
    ...
    return create_response(
        success=True, message="Analytics retrieved successfully", data=analytics.dict()
    )
...
async def get_scheduler_status():
    ...
    return create_response(
        success=True, message="Scheduler status retrieved successfully", data=status
    )
...
async def quick_excel_export(
    data: List[Dict[str, Any]], headers: List[str], filename: str = "export"
):
    """Quick data export to Excel"""

    try:
        filepath = os.path.join(
            REPORTS_DIR, f"{filename}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
