...
class ReportBuilder:
    ...
    def build_board_report(
        ...
        prev_incidents = financial_data.get(
            "previous_incidents", metrics.total_incidents
        )
        incident_reduction = (
            ((prev_incidents - metrics.total_incidents) / prev_incidents * 100)
            if prev_incidents > 0
            else 0
        )
        ...
        roi = (
            ((savings - incident_costs) / safety_investment * 100)
            if safety_investment > 0
            else 0
        )
    ...
    def _calculate_metrics(
        ...
        safety_score = max(
            0, base_score - incident_penalty - violation_penalty - near_miss_penalty
        )
        ...
        ppe_violations = sum(
            1
            for v in violations
            if "ppe" in v.get("type", "").lower()
            or "helmet" in v.get("type", "").lower()
        )
        total_ppe_checks = len(violations) + 50
        ppe_compliance = (
            ((total_ppe_checks - ppe_violations) / total_ppe_checks * 100)
    ...
    def _generate_executive_summary(
        self, metrics: ReportMetrics, start_date: datetime, end_date: datetime
    ) -> str:
        ...
        summary = (
            f"""This report covers a {period_days}-day period of safety performance. """
        )
    ...
    def _identify_key_findings(
        ...
        if metrics.fatalities > 0:
            findings.append(
                f"CRITICAL: {metrics.fatalities} fatal incident(s) occurred during this period"
            )

        if metrics.lost_time_incidents > 0:
            findings.append(
                f"{metrics.lost_time_incidents} lost-time incidents resulted in worker absence"
            )

        # High risk areas
        if metrics.high_risk_areas:
            findings.append(
                f"High-risk areas identified: {', '.join(metrics.high_risk_areas[:3])}"
            )
        ...
        if metrics.top_violations:
            ...
            findings.append(
                f"Most common violation: {top_type} ({top_count} occurrences)"
            )

        # Compliance
        if metrics.compliance_rate < 90:
            findings.append(
                f"Compliance rate of {metrics.compliance_rate:.1f}% is below target (90%)"
            )

        if metrics.ppe_compliance < 95:
            findings.append(
                f"PPE compliance at {metrics.ppe_compliance:.1f}% requires attention"
            )

        # Trends
        if metrics.trend_direction == "improving":
            findings.append(
                "Positive safety trend observed with declining incident rates"
            )
        elif metrics.trend_direction == "degrading":
            findings.append(
                "WARNING: Increasing incident trend requires immediate action"
            )

        return findings

    def _generate_recommendations(
        self, metrics: ReportMetrics, incidents: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if metrics.safety_score < 70:
            recommendations.append(
                "Implement comprehensive safety review and corrective action plan"
            )
            recommendations.append(
        ...
        if metrics.total_incidents > 10:
            recommendations.append(
                "Conduct root cause analysis for all incidents to identify systemic issues"
            )

        # Based on compliance
        if metrics.compliance_rate < 90:
            recommendations.append(
                "Enhance compliance monitoring with more frequent audits"
            )
        ...
        if metrics.ppe_compliance < 95:
            recommendations.append(
                "Implement strict PPE enforcement policy with consequences"
            )
            recommendations.append(
                "Review PPE availability and comfort to improve adoption"
            )

        # High risk areas
        if metrics.high_risk_areas:
            recommendations.append(
                f"Deploy additional safety measures in high-risk areas: {', '.join(metrics.high_risk_areas[:3])}"
            )

        # General
        recommendations.append("Continue monitoring safety metrics with weekly reviews")
        recommendations.append(
            "Recognize and reward teams with excellent safety records"
        )
    ...
    def _create_safety_tables(
        self, incidents: List[Dict[str, Any]], violations: List[Dict[str, Any]]
    ) -> List[ReportTable]:
        """Create data tables"""
    ...
    def _generate_board_summary(
        self, metrics: ReportMetrics, financial_data: Dict[str, Any]
    ) -> str:
        ...
        if metrics.safety_score >= 85:
            ...
            summary += (
                "satisfactory safety management with opportunities for improvement. "
            )
        else:
    ...
    def _identify_strategic_highlights(
        self, metrics: ReportMetrics, initiatives: Dict[str, Any]
    ) -> List[str]:
        ...
        if metrics.trend_direction == "improving":
            highlights.append(
                f"Safety performance improved with {metrics.trend_direction} incident trend"
            )
        ...
        if metrics.compliance_rate >= 95:
            highlights.append(
                f"Achieved excellent compliance rate of {metrics.compliance_rate:.1f}%"
            )
    ...
    def _generate_board_recommendations(
        ...
        if metrics.safety_score < 80:
            recommendations.append(
                "Approve additional budget allocation for safety improvements"
            )

        savings_ratio = financial_data.get("prevention_savings", 0) / max(
            financial_data.get("incident_costs", 1), 1
        )

        if savings_ratio > 2:
            recommendations.append(
                "Continue current safety investment levels - demonstrating strong ROI"
            )
        else:
            recommendations.append(
                "Increase preventive safety investments to improve cost-benefit ratio"
            )

        if metrics.trend_direction == "degrading":
            recommendations.append(
                "Authorize executive safety task force to address declining trends"
            )

        recommendations.append(
            "Review and approve updated safety policies for board adoption"
        )
    ...
    def _generate_compliance_recommendations(
        self, compliance_rate: float, violations: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if len(violations) > 10:
            recommendations.append(
                "Conduct root cause analysis of systemic compliance failures"
            )

        recommendations.append(
            "Update compliance training materials with recent findings"
        )
