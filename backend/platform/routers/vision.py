...
from backend.platform.utils_geometry import bbox_center, point_in_polygon
...
async def detect_image(
    ...
    zones_json: Optional[str] = Form(default=None),
    ...
    for obj in objects:
        ...
        for zone in zones:
            if zone.get("type") == "restricted" and point_in_polygon(
                (cx, cy), zone.get("polygon", [])
            ):
                violations.append(
                    {
                        "zone": zone.get("id", "restricted"),
    ...
    if violations:
        with get_conn() as conn:
            conn.execute(
                "INSERT INTO vision_events(event_type,camera_id,payload_json,created_at) VALUES(?,?,?,datetime('now'))",
                (
                    "zone_violation",
                    camera_id or 0,
...
def validate_zones(request: Request, payload: dict):
    ...
    inside = [
        z.get("id") for z in zones if point_in_polygon(point, z.get("polygon", []))
    ]
    ...
    with get_conn() as conn:
        rows = [
            ...
            for r in conn.execute(
                "SELECT * FROM vision_events ORDER BY id DESC LIMIT 50"
            ).fetchall()
        ]
