...
def create_ptw(body: PTWIn, request: Request):
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO ptw_permits(work_type,checklist_json,status,approver,created_at) VALUES(?,?,?,?,?)",
            (
                body.work_type,
        ...
        conn.commit()
    return ok(
        {"id": cur.lastrowid, "workflow": ["submitted", "approved", "closed"]},
...
def ptw_dashboard(request: Request):
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT status, COUNT(*) c FROM ptw_permits GROUP BY status"
        ).fetchall()
