...
def evaluate(request: Request):
    ...
    with get_conn() as conn:
        overdue = conn.execute(
            "SELECT COUNT(*) c FROM observations WHERE closed=0 AND due_date < ?",
            (today,),
        ).fetchone()["c"]
        high_risk = conn.execute(
            "SELECT COUNT(*) c FROM incidents WHERE risk_score >= 16"
        ).fetchone()["c"]
        env_breach = conn.execute(
            "SELECT COUNT(*) c FROM environment_measurements WHERE metric='PM2.5' AND value>35"
        ).fetchone()["c"]
    if overdue > 5:
    ...
    return ok(
        {"triggered": triggered, "channels": ["in_app", "email_stub", "webhook_stub"]},
