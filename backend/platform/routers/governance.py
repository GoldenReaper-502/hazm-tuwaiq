...
def create_policy(body: PolicyIn, request: Request):
    ...
    with get_conn() as conn:
        cur = conn.execute("INSERT INTO policies(site,name,description,created_at) VALUES(?,?,?,?)", (body.site, body.name, body.description, now))
        conn.execute("INSERT INTO audit_logs(actor,action,entity,entity_id,details_json,created_at) VALUES(?,?,?,?,?,?)", ("system", "create", "policy", str(cur.lastrowid), json.dumps(body.model_dump()), now))
        conn.commit()
...
def create_checklist(body: ChecklistIn, request: Request):
    now = datetime.now(timezone.utc).isoformat()
    with get_conn() as conn:
        cur = conn.execute("INSERT INTO compliance_checklists(site,item,status,owner,due_date,created_at) VALUES(?,?,?,?,?,?)", (body.site, body.item, body.status, body.owner, body.due_date, now))
        conn.commit()
    return ok({"id": cur.lastrowid}, request.state.request_id)
