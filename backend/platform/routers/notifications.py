...
def list_notifications(request: Request):
    with get_conn() as conn:
        rows = [dict(r) for r in conn.execute("SELECT * FROM notifications ORDER BY id DESC LIMIT 200").fetchall()]
...
def unread_count(request: Request):
    with get_conn() as conn:
        c = conn.execute("SELECT COUNT(*) c FROM notifications WHERE status='unread'").fetchone()["c"]
...
def mark_read(request: Request, payload: dict):
    ...
    with get_conn() as conn:
        cur = conn.execute(f"UPDATE notifications SET status='read' WHERE id IN ({placeholders})", ids)
        conn.commit()
