...
def status(request: Request):
    return ok({"version": "1.0.0", "time": datetime.now(timezone.utc).isoformat()}, request.state.trace_id)


@router.get("/metrics")
def metrics(request: Request):
    return ok({"requests_total": METRICS["requests_total"], "requests_by_path": dict(METRICS["requests_by_path"])}, request.state.trace_id)
