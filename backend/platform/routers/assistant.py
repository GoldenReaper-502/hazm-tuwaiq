...
def chat(body: ChatIn, request: Request):
    ...
    return ok(
        {
            "reply": answer,
            "recommendations": ["Toolbox talk", "Supervisor coaching", "PPE audit"],
        },
...
def daily_brief(request: Request):
    with get_conn() as conn:
        inc = conn.execute(
            "SELECT COUNT(*) c FROM incidents WHERE date(created_at)=date('now')"
        ).fetchone()["c"]
        obs = conn.execute(
            "SELECT COUNT(*) c FROM observations WHERE date(created_at)=date('now')"
        ).fetchone()["c"]
        alerts = conn.execute(
            "SELECT COUNT(*) c FROM alert_events WHERE date(created_at)=date('now')"
        ).fetchone()["c"]
    return ok(
        {
...
def recommendations(request: Request):
    return ok(
        {
            "items": [
                "Inspect high-risk work areas",
                "Close overdue corrective actions",
                "Review environmental exceedances",
            ]
        },
        request.state.request_id,
    )
    ...
    return ok(
        {
            ...
            "sections": ["Summary", "Risks", "Actions", "Recommendations"],
        },
        request.state.request_id,
    )
