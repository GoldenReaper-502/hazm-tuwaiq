...
from backend.platform.db import get_conn, init_db
    ...
    with get_conn() as conn:
        ...
        for u, p, fn, r, d, s in users:
            if not conn.execute(
                ...
                conn.execute(
                    "INSERT INTO users(username,password_hash,full_name,role,department,site) VALUES(?,?,?,?,?,?)",
                    (u, hash_password(p), fn, r, d, s),
                )

        if not conn.execute("SELECT 1 FROM sites").fetchone():
            conn.execute(
                "INSERT INTO sites(name,threshold_json) VALUES(?,?)",
                (
                    "Plant-1",
                    json.dumps(
                        {
        ...
        if not conn.execute("SELECT 1 FROM departments").fetchone():
            for dep, site in [
                ...
                conn.execute(
                    "INSERT INTO departments(name,site) VALUES(?,?)", (dep, site)
                )

        if not conn.execute("SELECT 1 FROM cameras").fetchone():
            cams = [
                ("Gate Cam", "Plant-1", "Gate-A", "online", "rtsp://demo/gate"),
                ("Warehouse Cam", "Plant-1", "WH-1", "offline", "rtsp://demo/wh1"),
            ]
            for c in cams:
                conn.execute(
        ...
        if not conn.execute("SELECT 1 FROM observations").fetchone():
            conn.execute(
                "INSERT INTO observations(category,location,site,observed_at,department,corrective_action,due_date,responsible_person,closed,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (
                    "unsafe act",
                    "Line-A",
                    "Plant-1",
        ...
        if not conn.execute("SELECT 1 FROM risks").fetchone():
            conn.execute(
                "INSERT INTO risks(hazard,site,department,controls,severity,likelihood,residual_risk,status,created_at) VALUES(?,?,?,?,?,?,?,?,?)",
                (
                    "Work at height",
                    "Plant-1",
                    "Maintenance",
        ...
        if not conn.execute("SELECT 1 FROM inspection_templates").fetchone():
            conn.execute(
                "INSERT INTO inspection_templates(name,checklist_json,created_at) VALUES(?,?,?)",
                (
                    "Daily HSE Walkdown",
                    json.dumps(["PPE", "Housekeeping", "Fire exits"]),
                    now_iso,
                ),
            )
        ...
        if not conn.execute("SELECT 1 FROM inspections").fetchone():
            conn.execute(
                "INSERT INTO inspections(site,department,template_id,scheduled_date,status,result_json,nonconformities_json,created_at) VALUES(?,?,?,?,?,?,?,?)",
                (
                    "Plant-1",
                    "Operations",
                    1,
        ...
        if not conn.execute("SELECT 1 FROM environment_measurements").fetchone():
            ...
            for m, v, u in env:
                exceeded = (
                    1 if (m == "noise" and v > 85) or (m == "CO2" and v > 1000) else 0
                )
                conn.execute(
                    "INSERT INTO environment_measurements(metric,value,unit,location,site,measured_at,exceeded) VALUES(?,?,?,?,?,?,?)",
                    (m, v, u, "Line-A", "Plant-1", now_iso, exceeded),
                )

        if not conn.execute("SELECT 1 FROM alert_events").fetchone():
            conn.execute(
                "INSERT INTO alert_events(rule_name,severity,payload_json,channel,created_at) VALUES(?,?,?,?,?)",
                (
                    "seed_alert",
                    "medium",
                    json.dumps({"msg": "demo"}),
        ...
        if not conn.execute("SELECT 1 FROM vision_events").fetchone():
            conn.execute(
                "INSERT INTO vision_events(event_type,camera_id,payload_json,created_at) VALUES(?,?,?,?)",
                (
                    "zone_violation",
                    1,
                    json.dumps({"class": "person", "zone": "restricted-1"}),
                    now_iso,
                ),
            )
        ...
        if not conn.execute("SELECT 1 FROM rules").fetchone():
            conn.execute(
                "INSERT INTO rules(name,condition_expr,action_expr,enabled,created_at) VALUES(?,?,?,?,?)",
                (
                    "Dust Threshold",
                    "dust>150",
                    "create_alert+action+notify",
                    1,
                    now_iso,
                ),
            )
