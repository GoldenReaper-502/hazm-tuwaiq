...
def export_to_pdf(
    reports: List[Dict[str, Any]], filename: Optional[str] = None
) -> Optional[bytes]:
    ...
    for report in reports:
        ...
        story.append(
            Paragraph(f"Timestamp: {report.get('timestamp')}", styles["Normal"])
        )
...
def export_to_excel(
    ...
    for report in reports:
        risk_level = report.get("risk_level", "UNKNOWN")
        risk_color = {"HIGH": "FFC7CE", "MEDIUM": "FFEB9C", "LOW": "C6EFCE"}.get(
            risk_level, "FFFFFF"
        )
        risk_font_color = {"HIGH": "9C0006", "MEDIUM": "9C6500", "LOW": "006100"}.get(
            risk_level, "000000"
        )
