...
class RBACManager:
    ...
    def get_user_permissions(self, user: User, roles_db: Dict[str, Role]) -> Set[str]:
        """
    ...
    def has_permission(
        self, user: User, permission_id: str, roles_db: Dict[str, Role]
    ) -> bool:
        """
    ...
    def check_permission(
        ...
        for perm_id in user_permissions:
            ...
            if (
                ...
                and perm.action == check.action
                and perm.scope == check.scope
            ):
                return True, None
    ...
    def assign_role(
        self, user: User, role_id: str, roles_db: Dict[str, Role]
    ) -> Tuple[bool, Optional[str]]:
        """
        Assign role to user
    ...
    def remove_role(self, user: User, role_id: str) -> Tuple[bool, Optional[str]]:
        """
...
class PermissionChecker:
    ...
    def require_permission(
        ...
        """
        check = PermissionCheck(
            user_id=user.id, resource=resource, action=action, scope=scope
        )
        ...
        if not has_perm:
            self.logger.warning(f"Permission denied for user {user.username}: {reason}")
