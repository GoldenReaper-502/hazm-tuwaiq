...
class EnhancedIntentAwareSafety:
    ...
    def update_worker_position(self, worker_id: str, position: Position):
    ...
    def _calculate_trajectory(self, worker_id: str) -> Trajectory:
        ...
        if len(positions) >= 3:
            ...
            # Time differences
            dt1 = (
                recent_positions[-1].timestamp - recent_positions[-2].timestamp
            ).total_seconds()
            dt2 = (
                recent_positions[-2].timestamp - recent_positions[-3].timestamp
            ).total_seconds()
        ...
        # Predict intent
        intent, confidence = self._predict_intent(positions, (vx, vy, vz), (ax, ay, az))
    ...
    def _predict_collisions(self, worker_id: str, trajectory: Trajectory):
        ...
        for equipment_id, equipment_pos in self.equipment_positions.items():
            collision_time = self._check_equipment_collision(
                future_positions, equipment_pos
            )
        ...
        for other_worker_id, other_trajectory in self.trajectories.items():
            ...
            collision_time = self._check_worker_collision(trajectory, other_trajectory)
    ...
    def _check_zone_collision(
        self, future_positions: List[Position], zone: SafetyZone
    ) -> Optional[float]:
        ...
        for i, pos in enumerate(future_positions):
            distance = np.sqrt(
                (pos.x - zx) ** 2 + (pos.y - zy) ** 2 + (pos.z - zz) ** 2
            )
    ...
    def _check_equipment_collision(
        self, future_positions: List[Position], equipment_pos: Position
    ) -> Optional[float]:
    ...
    def _check_worker_collision(
        self, trajectory1: Trajectory, trajectory2: Trajectory
    ) -> Optional[float]:
    ...
    def _calculate_collision_risk(
        self, time_to_collision: float, object_type: str
    ) -> CollisionRisk:
    ...
    def _recommend_intervention(
        self, risk_level: CollisionRisk, object_type: str
    ...
    def update_equipment_position(self, equipment_id: str, position: Position):
        """Update equipment position"""

        self.equipment_positions[equipment_id] = position

    def get_active_collision_predictions(
        self, min_risk_level: Optional[CollisionRisk] = None
    ...
    def get_system_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_collision_predictions": len(
                self.get_active_collision_predictions()
            ),
            "imminent_collisions": len(
                [
                    p
                    for p in self.get_active_collision_predictions()
            ...
            "average_prediction_confidence": (
                np.mean([t.confidence for t in self.trajectories.values()])
