...
class EnvironmentFusion:
    ...
    def fuse_environment(
        self, location: str, time_window: int = 60
    ) -> EnvironmentSnapshot:
        """
        ...
        snapshot = EnvironmentSnapshot(timestamp=datetime.now(), location=location)
    ...
    def _detect_anomalies(self, snapshot: EnvironmentSnapshot) -> List[str]:
        ...
        if snapshot.temperature is not None:
            if snapshot.temperature < self.thresholds["temperature"]["critical_min"]:
                anomalies.append(
                    f"Critical: Temperature too low ({snapshot.temperature:.1f}°C)"
                )
            elif snapshot.temperature > self.thresholds["temperature"]["critical_max"]:
                anomalies.append(
                    f"Critical: Temperature too high ({snapshot.temperature:.1f}°C)"
                )
            elif snapshot.temperature < self.thresholds["temperature"]["min"]:
                anomalies.append(
                    f"Warning: Temperature low ({snapshot.temperature:.1f}°C)"
                )
            elif snapshot.temperature > self.thresholds["temperature"]["max"]:
                anomalies.append(
                    f"Warning: Temperature high ({snapshot.temperature:.1f}°C)"
                )
        ...
        if snapshot.humidity is not None:
            if snapshot.humidity < self.thresholds["humidity"]["critical_min"]:
                anomalies.append(
                    f"Critical: Humidity too low ({snapshot.humidity:.1f}%)"
                )
            elif snapshot.humidity > self.thresholds["humidity"]["critical_max"]:
                anomalies.append(
                    f"Critical: Humidity too high ({snapshot.humidity:.1f}%)"
                )
        ...
        if snapshot.noise_level is not None:
            if snapshot.noise_level > self.thresholds["noise"]["critical"]:
                anomalies.append(
                    f"Critical: Dangerous noise level ({snapshot.noise_level:.1f} dB)"
                )
            elif snapshot.noise_level > self.thresholds["noise"]["max"]:
                anomalies.append(
                    f"Warning: High noise level ({snapshot.noise_level:.1f} dB)"
                )
        ...
        if snapshot.air_quality_index is not None:
            if snapshot.air_quality_index > self.thresholds["air_quality"]["hazardous"]:
                anomalies.append(
                    f"Critical: Hazardous air quality (AQI: {snapshot.air_quality_index:.0f})"
                )
            elif (
                snapshot.air_quality_index > self.thresholds["air_quality"]["unhealthy"]
            ):
                anomalies.append(
                    f"Warning: Unhealthy air quality (AQI: {snapshot.air_quality_index:.0f})"
                )
        ...
        if snapshot.light_level is not None:
            if snapshot.light_level < self.thresholds["light"]["min"]:
                anomalies.append(
                    f"Warning: Insufficient lighting ({snapshot.light_level:.0f} lux)"
                )
        ...
        if snapshot.vibration_level is not None:
            if snapshot.vibration_level > self.thresholds["vibration"]["critical"]:
                anomalies.append(
                    f"Critical: Excessive vibration ({snapshot.vibration_level:.1f} mm/s)"
                )
            elif snapshot.vibration_level > self.thresholds["vibration"]["max"]:
                anomalies.append(
                    f"Warning: High vibration ({snapshot.vibration_level:.1f} mm/s)"
                )
    ...
    def _calculate_safety_score(self, snapshot: EnvironmentSnapshot) -> float:
        ...
        if snapshot.temperature is not None:
            if (
                snapshot.temperature < self.thresholds["temperature"]["critical_min"]
                or snapshot.temperature > self.thresholds["temperature"]["critical_max"]
            ):
                penalties.append(30)
            elif (
                snapshot.temperature < self.thresholds["temperature"]["min"]
                or snapshot.temperature > self.thresholds["temperature"]["max"]
            ):
                penalties.append(10)
        ...
        if snapshot.humidity is not None:
            if (
                snapshot.humidity < self.thresholds["humidity"]["critical_min"]
                or snapshot.humidity > self.thresholds["humidity"]["critical_max"]
            ):
                penalties.append(20)
            elif (
                snapshot.humidity < self.thresholds["humidity"]["min"]
                or snapshot.humidity > self.thresholds["humidity"]["max"]
            ):
                penalties.append(5)
        ...
        if snapshot.air_quality_index is not None:
            if snapshot.air_quality_index > self.thresholds["air_quality"]["hazardous"]:
                ...
                snapshot.air_quality_index > self.thresholds["air_quality"]["moderate"]
            ):
                penalties.append(10)
    ...
    def detect_environmental_changes(
        self, location: str, lookback_minutes: int = 30
    ) -> List[Dict[str, Any]]:
        ...
        recent_snapshots = [
            s
            for s in self.environment_history
                ...
                changes.append(
                    {
                        ...
                        "change": temp_change,
                        "significance": (
                            "high" if abs(temp_change) >= 10 else "moderate"
                        ),
                ...
                changes.append(
                    {
                        ...
                        "change": noise_change,
                        "significance": (
                            "high" if abs(noise_change) >= 20 else "moderate"
                        ),
    ...
    def predict_environmental_risk(
        self, location: str, forecast_minutes: int = 30
    ) -> Dict[str, Any]:
        """Predict future environmental risk based on trends"""

        # Get recent history
        recent_snapshots = [
            s for s in self.environment_history[-20:] if s.location == location
        ]

        if len(recent_snapshots) < 3:
            return {"predicted_risk": "unknown", "confidence": 0.0, "factors": []}

        # Analyze trends
        temps = [s.temperature for s in recent_snapshots if s.temperature is not None]
        noise_levels = [
            s.noise_level for s in recent_snapshots if s.noise_level is not None
        ]
        aqi_values = [
            s.air_quality_index
            for s in recent_snapshots
            if s.air_quality_index is not None
        ]
    ...
    def _generate_environmental_recommendation(
            ...
            health_report["sensor_details"].append(
                {
                    ...
                    "status": status,
                    "last_reading_minutes_ago": (
                        (now - last_reading).total_seconds() / 60
                        if last_reading
                        else None
                    ),
                }
