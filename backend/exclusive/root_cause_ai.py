...
class RootCause:
    ...
    def __init__(
        self, cause_id: str, primary_cause: str, category: str, confidence: float
    ):
    ...
    def calculate_roi(self) -> float:
        ...
        if self.prevention_cost_estimate == 0:
            return 0.0
        return (
            self.potential_loss_prevention - self.prevention_cost_estimate
        ) / self.prevention_cost_estimate
...
class RootCauseAI:
    ...
    def analyze_incident(self, incident: Dict[str, Any], depth: int = 5) -> RootCause:
        """
        ...
        root_cause = self._synthesize_findings(
            incident, five_whys, fishbone, patterns, barriers
        )
    ...
    def _five_whys_analysis(
        self, incident: Dict[str, Any], depth: int
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _infer_why(self, issue: str, incident: Dict[str, Any], level: int) -> str:
    ...
    def _fishbone_analysis(
        ...
        if incident.get("worker_id"):
            fishbone["people"].append(
                CausalFactor("human_error", "Worker action or inaction", 0.6)
            )

        if incident.get("training_status") == "incomplete":
            fishbone["people"].append(
                CausalFactor("training_gap", "Insufficient or incomplete training", 0.8)
            )

        # Equipment factors
        if "equipment" in incident_type.lower() or "machine" in incident_type.lower():
            fishbone["equipment"].append(
                CausalFactor(
                    "equipment_failure", "Equipment malfunction or degradation", 0.7
                )
            )
        ...
        if conditions.get("temperature"):
            ...
            if temp > 35 or temp < 5:
                fishbone["environment"].append(
                    CausalFactor(
                        "extreme_conditions", f"Extreme temperature ({temp}°C)", 0.75
                    )
                )

        # Process factors
        if incident.get("procedure_followed") is False:
            fishbone["process"].append(
                CausalFactor(
                    "process_deviation", "Standard procedure not followed", 0.85
                )
            )

        # Management factors
        if incident.get("supervision") == "absent":
            fishbone["management"].append(
                CausalFactor("supervision_gap", "Lack of adequate supervision", 0.7)
            )
    ...
    def _detect_location_pattern(
        ...
        location_count = sum(
            1
            for inc in self.incident_database[-100:]
    ...
    def _estimate_impact(self, root_cause: RootCause):
        ...
        total_cost = sum(
            rec.get("estimated_cost", 0) for rec in root_cause.recommended_actions
        )
        ...
        impact_multiplier = {"low": 1.0, "medium": 3.0, "high": 10.0, "critical": 50.0}
        ...
        root_cause.potential_loss_prevention = (
            base_incident_cost * estimated_prevented * multiplier
        )
    ...
    def get_top_root_causes(self, limit: int = 10) -> List[RootCause]:
        ...
        causes.sort(
            key=lambda x: (x.systemic_impact == "high", x.calculate_roi()), reverse=True
        )
    ...
    def generate_executive_report(self) -> Dict[str, Any]:
        ...
        total_savings = sum(
            rc.potential_loss_prevention for rc in self.identified_root_causes.values()
        )

        total_investment = sum(
            rc.prevention_cost_estimate for rc in self.identified_root_causes.values()
        )
        ...
        return {
            ...
            "projected_roi": (
                (total_savings - total_investment) / total_investment
