...
class PredictiveMaintenance:
    ...
    def record_maintenance(self, record: MaintenanceRecord):
        ...
        if equipment:
            ...
            if record.maintenance_type == "preventive":
                ...
                equipment.health_status = self._get_health_status(
    ...
    def predict_failures(
        ...
        predictions.sort(
            key=lambda p: p.days_until_failure * (1 - p.failure_probability)
        )

        return predictions

    def _predict_single_failure(
        ...
        if recent_sensors:
            ...
            if anomaly_rate > 0.1:  # More than 10% anomalies
                ...
                factors.append(
                    f"Sensor anomalies ({anomaly_rate*100:.0f}% of readings)"
                )
        ...
        if age_factor > 0.7:  # Over 70% of expected life
            ...
            factors.append(
                f"Age ({age_years:.1f} years, {age_factor*100:.0f}% of expected life)"
            )
        ...
        if equipment.last_maintenance:
            ...
            if days_since_maintenance > expected_interval * 1.2:  # 20% overdue
                maintenance_factor = (
                    days_since_maintenance - expected_interval
                ) / expected_interval
                failure_probability += min(0.3, maintenance_factor * 0.1)
                factors.append(
                    f"Maintenance overdue ({days_since_maintenance} days since last)"
                )
        else:
            # Never maintained
        ...
        if equipment.mtbf and equipment.operating_hours > equipment.mtbf * 0.9:
            ...
            factors.append(
                f"Approaching MTBF ({equipment.operating_hours:.0f} hrs vs {equipment.mtbf:.0f} hrs)"
            )
        ...
        days_until_failure = int(
            base_days * (1 - failure_probability) * (equipment.health_score / 100.0)
        )
        ...
        cost_if_ignored = base_repair_cost * (
            1.5 + safety_factor * 0.2
        )  # Higher for safety-critical
        ...
        recommended_actions = self._generate_maintenance_recommendations(
            equipment, failure_probability, days_until_failure
        )
    ...
    def _determine_failure_mode(
        self, equipment: Equipment, recent_sensors: List[SensorData]
    ) -> str:
    ...
    def _generate_maintenance_recommendations(
        self, equipment: Equipment, failure_probability: float, days_until_failure: int
    ) -> List[str]:
        ...
        if days_until_failure < 7:
            ...
            recommendations.append(
                "Consider taking equipment out of service immediately"
            )
        ...
        if equipment.health_score < 50:
            ...
            recommendations.append(
                "Consider replacement if repair cost > 60% of new equipment"
            )
    ...
    def _update_reliability_metrics(self, equipment_id: str):
        ...
        records = [
            r for r in self.maintenance_history if r.equipment_id == equipment_id
        ]
        ...
        failure_records = [
            r for r in records if r.maintenance_type in ["corrective", "emergency"]
        ]
            ...
            prioritized.append(
                {
                    ...
                    + timedelta(days=min(7, pred.days_until_failure // 2)),
    ...
    def get_fleet_health_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            + status_counts[HealthStatus.POOR.value],
            "predictions_count": len(predictions),
            "urgent_maintenance_needed": len(critical_predictions),
            "total_estimated_savings": sum(
                p.roi * p.estimated_preventive_cost for p in predictions
            ),
