...
class IntelligentComplianceDrift:
    ...
    def __init__(self):
        ...
        self.observation_windows: Dict[str, deque] = defaultdict(
    ...
    def _detect_drift(self, observation: ComplianceObservation):
        ...
        affected_workers = list(
            set([obs.worker_id for obs in recent_obs if obs.worker_id])
        )
        ...
        interventions = self._generate_interventions(
            observation.category, drift_magnitude, severity, root_causes
        )
        ...
        existing_drift = next(
            (
                ...
                if d.category == observation.category
                and d.requirement == observation.requirement
    ...
    def _estimate_drift_start(self, window: deque, baseline_level: float) -> datetime:
        """Estimate when drift started"""
    ...
    def _identify_root_causes(
        self, recent_obs: List[ComplianceObservation], baseline: ComplianceBaseline
    ) -> List[str]:
        ...
        if times_of_day:
            ...
            if times_of_day.count(most_common_hour) / len(times_of_day) > 0.6:
                root_causes.append(
                    f"Drift concentrated around hour {most_common_hour}:00 (shift fatigue or supervision gap)"
                )
        ...
        if locations:
            ...
            if locations.count(most_common_location) / len(locations) > 0.5:
                root_causes.append(
                    f"Drift concentrated in {most_common_location} (local culture issue)"
                )
        ...
        if workers:
            ...
            if unique_workers < len(workers) * 0.3:  # Same workers repeatedly
                root_causes.append(
                    "Drift involves small group of repeat offenders (targeted training needed)"
                )
        ...
        if len(compliance_levels) >= 3:
            ...
            if abs(trend) > 0.01:  # Significant slope
                root_causes.append(
                    "Gradual drift pattern (normalization of deviance - systemic issue)"
                )
            else:
                root_causes.append(
                    "Sudden drift pattern (recent policy change or leadership change)"
                )
    ...
    def _generate_interventions(
        ...
        if severity in [DriftSeverity.SEVERE, DriftSeverity.SIGNIFICANT]:
            ...
            interventions.append(
                "Reset compliance expectations with clear consequences"
            )
        elif severity == DriftSeverity.MODERATE:
            interventions.append(
                "Schedule compliance reinforcement training within 48 hours"
            )
        ...
        for cause in root_causes:
            if "shift fatigue" in cause.lower():
                ...
                interventions.append(
                    "Implement one-on-one coaching for repeat offenders"
                )
            elif "normalization" in cause.lower():
                interventions.append(
                    "Reset safety culture with visible leadership commitment"
                )
    ...
    def _calculate_drift_confidence(
        self, window: deque, baseline: ComplianceBaseline
    ) -> float:
        ...
        if len(recent_obs) >= 5:
            ...
            decreasing_count = sum(
                1
                for i in range(1, len(compliance_levels))
        ...
        confidence = (
            observation_factor * 0.4 + variance_factor * 0.3 + consistency_factor * 0.3
        )
    ...
    def get_active_drifts(
        ...
        if min_severity:
            ...
            drifts = [
                d for d in drifts if severity_order.index(d.severity) >= min_index
            ]
        ...
        drifts.sort(
            key=lambda d: (severity_scores[d.severity], d.drift_magnitude), reverse=True
        )
    ...
    def get_compliance_health(self) -> Dict[str, Any]:
        ...
        for key, baseline in self.baselines.items():
            ...
            if window:
                recent_compliance = np.mean(
                    [obs.actual_compliance for obs in list(window)[-20:]]
                )
        ...
        drift_counts = {
            "severe": sum(
                1 for d in self.detected_drifts if d.severity == DriftSeverity.SEVERE
            ),
            "significant": sum(
                1
                for d in self.detected_drifts
                if d.severity == DriftSeverity.SIGNIFICANT
            ),
            "moderate": sum(
                1 for d in self.detected_drifts if d.severity == DriftSeverity.MODERATE
            ),
            "minor": sum(
                1 for d in self.detected_drifts if d.severity == DriftSeverity.MINOR
            ),
        }
    ...
    def _identify_at_risk_categories(self) -> List[str]:
        ...
        sorted_categories = sorted(
            category_drift_scores.items(), key=lambda x: x[1], reverse=True
        )
