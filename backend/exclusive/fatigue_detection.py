...
class AdvancedFatigueDetection:
    ...
    def __init__(self):
        ...
        self.circadian_multipliers = {
            ...
            1: 1.6,
            2: 1.7,
            3: 1.8,  # Night hours - high fatigue
            4: 1.6,
            5: 1.4,
            6: 1.2,
            7: 1.0,
            ...
            9: 0.8,
            10: 0.8,
            11: 0.9,  # Morning - low fatigue
            12: 1.0,
            13: 1.1,
            14: 1.3,
            15: 1.2,  # Post-lunch dip
            16: 1.0,
            17: 0.9,
            18: 1.0,
            19: 1.1,
            ...
            21: 1.3,
            22: 1.4,
            23: 1.5,  # Evening - increasing fatigue
        }
    ...
    def assess_fatigue(self, worker_id: str) -> FatigueAssessment:
        ...
        contributing_factors = self._identify_contributing_factors(
            recent_signals, worker_id, now
        )

        # Calculate confidence
        confidence = min(
            1.0, len(recent_signals) / 10.0
        ...
        recommended_actions = self._generate_fatigue_recommendations(
            fatigue_level, contributing_factors, worker_id
        )
    ...
    def _calculate_physiological_score(self, signals: List[FatigueSignal]) -> float:
        ...
        relevant_signals = [
            s for s in signals if s.indicator_type in physiological_indicators
        ]
    ...
    def _calculate_behavioral_score(self, signals: List[FatigueSignal]) -> float:
        ...
        relevant_signals = [
            s for s in signals if s.indicator_type in behavioral_indicators
        ]
    ...
    def _get_time_since_last_break(self, worker_id: str) -> float:
        ...
        if (
            worker_id not in self.work_shifts
            or not self.work_shifts[worker_id].breaks_taken
        ):
            # No breaks, time since shift start
            if worker_id in self.work_shifts:
                return (
                    datetime.now() - self.work_shifts[worker_id].shift_start
                ).total_seconds() / 60.0
            return 0.0
    ...
    def _identify_contributing_factors(
        self, signals: List[FatigueSignal], worker_id: str, now: datetime
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_accident_risk(
        self, fatigue_score: float, fatigue_level: FatigueLevel
    ) -> float:
        ...
        for factor in contributing_factors[:3]:  # Top 3 factors
            if factor["factor"] == "extended_work_duration":
                ...
                recommendations.append(
                    "Consider task rotation during circadian low periods"
                )
    ...
    def get_team_fatigue_report(self, worker_ids: List[str]) -> Dict[str, Any]:
        ...
        if not assessments:
            return {"team_size": 0, "average_fatigue": 0.0, "critical_count": 0}
        ...
        high_risk = [
            ...
            if a.fatigue_level
            in [FatigueLevel.HIGH, FatigueLevel.SEVERE, FatigueLevel.CRITICAL]
        ]
        ...
        return {
            ...
            "team_recommendation": self._generate_team_recommendation(
                avg_fatigue, high_risk
            ),
        }

    def _generate_team_recommendation(
        self, avg_fatigue: float, high_risk_workers: List[Dict]
    ) -> str:
        ...
        if len(high_risk_workers) >= 3 or avg_fatigue > 60:
            ...
            return (
                "Team fatigue levels moderate. Ensure break schedules are maintained."
            )
        else:
