...
class BehavioralPatternRecognition:
    ...
    def __init__(self):
        ...
        self.behavior_sequences: Dict[str, List[BehaviorObservation]] = defaultdict(
            list
        )
    ...
    def _update_worker_profile(self, observation: BehaviorObservation):
        ...
        elif observation.category in [
            BehaviorCategory.UNSAFE,
            BehaviorCategory.AT_RISK,
        ]:
        ...
        profile.average_risk_score = (
            alpha * observation.risk_score + (1 - alpha) * profile.average_risk_score
        )
    ...
    def _analyze_worker_strengths_concerns(self, worker_id: str):
        ...
        recent_obs = [
            obs for obs in self.behavior_sequences[worker_id] if obs.timestamp >= cutoff
        ]
    ...
    def _detect_patterns(self, worker_id: str):
        ...
        cutoff = datetime.now() - timedelta(
            hours=self.pattern_thresholds["time_window_hours"]
        )
        recent_obs = [
            obs for obs in self.behavior_sequences[worker_id] if obs.timestamp >= cutoff
        ]
        ...
        unsafe_obs = [
            obs
            for obs in recent_obs
            if obs.category in [BehaviorCategory.UNSAFE, BehaviorCategory.VIOLATION]
        ]
        if len(unsafe_obs) >= 3:
            ...
            for hour, scores in hour_risk.items():
                if len(scores) >= 2 and np.mean(scores) > 60:
                    self._create_pattern(
                        ...
                        observations=[
                            obs for obs in recent_obs if obs.timestamp.hour == hour
                        ],
                        risk_level="moderate",
        ...
        rushing_obs = [
            obs for obs in recent_obs if obs.category == BehaviorCategory.RUSHING
        ]
        ...
        if existing:
            ...
            if worker_id in self.worker_profiles:
                if (
                    ...
                    not in self.worker_profiles[worker_id].risk_patterns
                ):
                    self.worker_profiles[worker_id].risk_patterns.append(
                        pattern.pattern_type
                    )

    def _extract_predictive_indicators(
        self, observations: List[BehaviorObservation]
    ) -> List[str]:
    ...
    def predict_next_risk(self, worker_id: str) -> Dict[str, Any]:
        ...
        most_likely_behavior = (
            Counter(recent_types).most_common(1)[0][0].value
            if recent_types
            else "unknown"
        )
        ...
        return {
            ...
            "contributing_patterns": [
                p.pattern_type for p in self.patterns if p.worker_id == worker_id
            ],
        }
    ...
    def get_team_risk_analysis(self, team_ids: List[str]) -> Dict[str, Any]:
        ...
        team_profiles = [
            self.worker_profiles[wid] for wid in team_ids if wid in self.worker_profiles
        ]
        ...
        return {
            ...
            "recommendation": self._generate_team_recommendation(
                team_avg_risk, high_risk, team_concerns
            ),
        }
