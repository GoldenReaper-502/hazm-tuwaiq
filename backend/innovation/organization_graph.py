            ...
            if rule["type"] == "mandatory":
                if not self._check_mandatory(rule, context):
                    return False, rule.get(
                        "violation_message", "Constitutional violation"
                    )
...
class OrganizationGraph:
    ...
    def _init_default_roles(self):
        ...
        manager.add_dynamic_rule(
            {"condition": "risk_level == 'CRITICAL'", "grant": ["OVERRIDE_AI"]}
        )
    ...
    def find_responsible_persons(
        self, incident_location: str, risk_level: RiskLevel
    ) -> List[Dict]:
        """
        ...
        responsible.sort(
            key=lambda x: authority_order.get(x["authority"], 0), reverse=True
        )
    ...
    def check_permission(
        self, person_id: str, permission: Permission, context: Dict
    ) -> bool:
    ...
    def validate_action(
        self, action: str, person_id: str, context: Dict
