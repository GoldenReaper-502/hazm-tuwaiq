...
class WorkerRiskProfiling:
    ...
    def suggest_task_redistribution(self, redistribution_data: Dict[str, Any]) -> Dict:
        ...
        return {
            ...
            "risk_reduction": self._calculate_risk_reduction(
                redistribution_data, assignments
            ),
        ...
        if worker_id:
            ...
            return {
                ...
                "behavior_ratio": round(
                    profile.safe_behavior_count
                    / max(profile.safe_behavior_count + profile.violation_count, 1),
                    2,
                ),
            ...
            return {
                ...
                "risk_distribution": {
                    "high": round(
                        len([s for s in risk_scores if s > 70])
                        / len(risk_scores)
                        * 100,
                        2,
                    ),
                    "medium": round(
                        len([s for s in risk_scores if 40 <= s <= 70])
                        / len(risk_scores)
                        * 100,
                        2,
                    ),
                ...
                "total_violations": sum(
                    p.violation_count for p in self.worker_profiles.values()
                ),
                "total_safe_behaviors": sum(
                    p.safe_behavior_count for p in self.worker_profiles.values()
                ),
    ...
    def _detect_behavior_pattern(
        self, profile: WorkerProfile, behavior_data: Dict
    ) -> Dict:
        """اكتشاف نمط السلوك"""
        ...
        return {"pattern_detected": pattern_detected, "description": description}
    ...
    def _generate_worker_recommendations(self, profile: WorkerProfile) -> List[str]:
        ...
        if profile.risk_score > 70:
            recommendations.extend(
                ["تدريب تصحيحي فوري", "مراقبة مكثفة", "مراجعة إجراءات السلامة"]
            )
        elif profile.risk_score > 40:
            recommendations.extend(["تدريب إضافي", "متابعة دورية"])
        else:
    ...
    def _get_task_recommendations(self, task_type: str, risk_score: float) -> List[str]:
        ...
        if risk_score > 70:
            recommendations.extend(
                ["تعيين عامل ذو خبرة عالية", "إشراف مكثف", "فحص المعدات قبل البدء"]
            )
        else:
    ...
    def _get_time_based_recommendations(self, hour: int, shift_type: str) -> List[str]:
        ...
        if 22 <= hour or hour <= 4:
            recommendations.extend(
                ["زيادة الإضاءة", "فترات راحة أكثر", "مراقبة اليقظة"]
            )
        elif 13 <= hour <= 15:
    ...
    def _get_fatigue_recommendations(self, fatigue_score: float) -> List[str]:
        ...
        if fatigue_score > 70:
            return ["راحة فورية ضرورية", "عدم تكليف بمهام خطرة", "فحص طبي"]
        elif fatigue_score > 50:
            return ["تقليل ساعات العمل", "زيادة فترات الراحة"]
        else:
    ...
    def _optimize_task_assignment(self, workers: List[Dict], tasks: List[Dict]) -> Dict:
        ...
        sorted_tasks = sorted(
            tasks, key=lambda t: t.get("risk_level", 50), reverse=True
        )
    ...
    def _analyze_worker_trend(self, profile: WorkerProfile) -> str:
        """تحليل اتجاه العامل"""
        ratio = profile.safe_behavior_count / max(
            profile.safe_behavior_count + profile.violation_count, 1
        )
