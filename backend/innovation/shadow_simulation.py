...
class ShadowSimulator:
    ...
    def run_shadow_simulation(
        self, actual_event: Dict, alternative_scenarios: List[str]
    ) -> Dict:
        """تشغيل محاكاة ظل"""
        ...
        results["value_saved"] = self._calculate_value_saved(
            actual_event, results["alternative_scenarios"]
        )
    ...
    def _simulate_scenario(self, scenario: str, actual_event: Dict) -> Dict:
        ...
        consequences = self._estimate_consequences(
            simulated_risk, simulated_severity, incident_would_occur, actual_event
        )
    ...
    def _estimate_consequences(
        self, risk: float, severity: float, probability: float, context: Dict
    ) -> Dict:
        ...
        total_cost = (
            base_cost + injury_cost + downtime_cost + legal_cost
        ) * probability
    ...
    def _calculate_value_saved(self, actual: Dict, alternatives: List[Dict]) -> Dict:
        ...
        worst_scenario = max(
            alternatives,
            key=lambda x: x.get("estimated_consequences", {})
            .get("financial_cost", {})
            .get("estimated_total_sar", 0),
        )

        worst_cost = (
            worst_scenario.get("estimated_consequences", {})
            .get("financial_cost", {})
            .get("estimated_total_sar", 0)
        )
        worst_injuries = (
            worst_scenario.get("estimated_consequences", {})
            .get("human_cost", {})
            .get("estimated_victims", 0)
        )

        return {
            "financial_value_saved_sar": worst_cost,
            "injuries_prevented": worst_injuries,
            "downtime_avoided_hours": worst_scenario.get("estimated_consequences", {})
            .get("operational_impact", {})
            .get("estimated_downtime_hours", 0),
    ...
    def generate_what_if_report(self, simulation_id: str, language: str = "ar") -> Dict:
        """توليد تقرير "ماذا لو"""
        sim = next(
            (s for s in self.simulations if s["simulation_id"] == simulation_id), None
        )
    ...
    def get_saved_value_summary(self, days: int = 30) -> Dict:
        ...
        total_financial = sum(
            s["value_saved"].get("financial_value_saved_sar", 0) for s in recent_sims
        )

        total_injuries = sum(
            s["value_saved"].get("injuries_prevented", 0) for s in recent_sims
        )

        total_downtime = sum(
            s["value_saved"].get("downtime_avoided_hours", 0) for s in recent_sims
        )
        ...
        return {
            ...
            "average_value_per_intervention": (
                total_financial // len(recent_sims) if recent_sims else 0
            ),
...
class SafetyBudgetOptimizer:
    ...
    def _suggest_actions(self, area: Dict, budget: float) -> List[Dict]:
        ...
        if budget >= 10000:
            actions.append(
                {"action": "تدريب إضافي للعمال", "cost": 10000, "risk_reduction": 0.15}
            )

        if budget >= 20000:
            actions.append(
                {"action": "تحديث معدات السلامة", "cost": 20000, "risk_reduction": 0.25}
            )

        # إجراءات متوسطة (50-100 ألف)
        if budget >= 50000:
            actions.append(
                {"action": "نظام مراقبة متقدم", "cost": 50000, "risk_reduction": 0.35}
            )
    ...
    def _generate_recommendations(self, allocations: List[Dict]) -> List[str]:
        ...
        if high_risk_areas:
            recommendations.append(
                f"الأولوية القصوى: {len(high_risk_areas)} منطقة عالية الخطورة تحتاج تدخل فوري"
            )
        ...
        if total_budget < 100000:
            recommendations.append(
                "ميزانية السلامة منخفضة - يُنصح بالزيادة لتحقيق معايير أعلى"
            )
