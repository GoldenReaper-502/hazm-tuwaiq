...
class MicroBehaviorDetector:
    ...
    def _detect_anomalies(
        self, worker_id: str, indicators: List[str], context: Dict
    ) -> List[Dict]:
        """الكشف عن الأنماط الشاذة"""
        detected = []
                ...
                detected.append(
                    {
                        ...
                        "matches": matches,
                        "indicators": [
                            ind for ind in indicators if ind in pattern["indicators"]
                        ],
                        "risk_weight": pattern["risk_weight"],
                        "severity": (
                            "high" if matches > pattern["threshold"] else "medium"
                        ),
                    }
                )
    ...
    def _check_repetitive_errors(self, worker_id: str, context: Dict) -> Optional[Dict]:
        ...
        recent = [
            h
            for h in self.behavior_history[worker_id]
