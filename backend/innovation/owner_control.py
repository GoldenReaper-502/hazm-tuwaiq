...
class OwnerControlCenter:
    ...
    def create_role(self, owner_id: str, role_data: Dict) -> tuple[bool, str]:
        ...
        try:
            ...
            self._log_action(
                owner_id, "create_role", {"role_id": role.id, "role_name": role.name}
            )
    ...
    def modify_role(
        self, owner_id: str, role_id: str, modifications: Dict
    ) -> tuple[bool, str]:
        """تعديل دور موجود"""
        if not self._verify_owner(owner_id):
        ...
        try:
            ...
            self._log_action(
                owner_id,
                "modify_role",
                {"role_id": role_id, "modifications": modifications},
            )
    ...
    def delete_role(self, owner_id: str, role_id: str) -> tuple[bool, str]:
        ...
        users_with_role = [
            n
            for n in organization_graph.nodes.values()
        ...
        if users_with_role:
            return (
                False,
                f"Cannot delete role: {len(users_with_role)} users still assigned",
            )
    ...
    def set_constitutional_rule(self, owner_id: str, rule: Dict) -> tuple[bool, str]:
        ...
        try:
            ...
            self._log_action(owner_id, "add_constitutional_rule", {"rule": rule})
    ...
    def configure_ai_authority(
        self, owner_id: str, level: str, settings: Dict
    ) -> tuple[bool, str]:
        ...
        self._log_action(
            owner_id, "configure_ai_authority", {"level": level, "settings": settings}
        )

        return True, f"AI authority configured to '{level}'"

    def override_ai_decision(
        self, owner_id: str, decision_id: str, reason: str
    ) -> tuple[bool, str]:
    ...
    def create_person(self, owner_id: str, person_data: Dict) -> tuple[bool, str]:
        ...
        try:
            ...
            self._log_action(
                owner_id, "create_person", {"person_id": person.id, "name": person.name}
            )
    ...
    def assign_role(
        self, owner_id: str, person_id: str, role_id: str
    ) -> tuple[bool, str]:
        """تعيين دور لشخص"""
        if not self._verify_owner(owner_id):
            return False, "Unauthorized"
        ...
        self._log_action(
            owner_id,
            "assign_role",
            {"person_id": person_id, "old_role": old_role, "new_role": role_id},
        )
    ...
    def get_audit_log(self, owner_id: str, filters: Dict = None) -> List[Dict]:
        ...
        if filters:
            ...
            if "start_date" in filters:
                ...
                logs = [
                    l for l in logs if datetime.fromisoformat(l["timestamp"]) >= start
                ]
    ...
    def get_system_overview(self, owner_id: str) -> Dict:
        ...
        locations = [
            n for n in organization_graph.nodes.values() if n.type == "location"
        ]
