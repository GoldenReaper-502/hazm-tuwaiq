...
class DigitalSafetyTwin:
        ...
        results = {
            ...
            "roi_of_prevention": (
                round(
                    ...
                    / impact["prevention_cost"]
                    * 100,
                    2,
                )
                if impact["prevention_cost"] > 0
                else 0
            ),
        }
    ...
    def test_procedure_change(
        self, current_procedures: Dict, new_procedures: Dict
    ) -> Dict:
        ...
        improvement_percent = (
            (improvement / current_risk * 100) if current_risk > 0 else 0
        )
        ...
        return {
            ...
            "recommendation": (
                "ننصح بالتطبيق" if improvement > 0 else "لا ننصح بالتطبيق"
            ),
            "affected_areas": self._identify_affected_areas(
                current_procedures, new_procedures
            ),
    ...
    def generate_virtual_heatmap(self, timeframe: str = "current") -> Dict:
        ...
        return {
            ...
            "zones_requiring_attention": len(
                [d for d in heatmap_data if d["risk_level"] > 70]
            ),
        }
    ...
    def _calculate_initial_risk(self, location_config: Dict) -> float:
        ...
        danger_count = sum(
            1 for eq in equipment if any(d in eq.lower() for d in dangerous_equipment)
        )
    ...
    def _simulate_impact(
        self, scenario: SafetyScenario, location_id: str, changes: Dict
    ) -> Dict:
        """محاكاة تأثير الحادث"""
    ...
    def _get_nearby_locations(
        self, location_id: str, radius: float = 50.0
    ) -> List[str]:
    ...
    def _calculate_heatmap_risk(
        self, location: WorksiteLocation, timeframe: str
    ) -> float:
    ...
    def _recommend_preventive_actions(
        self, location: WorksiteLocation, future_risk: float
    ) -> List[str]:
