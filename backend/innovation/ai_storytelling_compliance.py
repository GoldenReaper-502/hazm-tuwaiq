...
"""
    ...
    def _analyze_root_causes(self, incident: Dict) -> Dict:
        ...
        return {
            "primary_cause": (
                incident.get("root_causes", [])[0]
                if incident.get("root_causes")
                else "غير محدد"
            ),
            "contributing_factors": (
                incident.get("root_causes", [])[1:]
                if len(incident.get("root_causes", [])) > 1
                else []
            ),
    ...
    def _create_prevention_guide(self, incident: Dict) -> Dict:
        ...
        return {
            ...
            "monitoring": ["تفتيش يومي", "تقارير أسبوعية", "مراجعة شهرية"],
        }
...
class ComplianceAutoAuditor:
    ...
    def conduct_audit(self, site_data: Dict, standard: str = "ISO45001") -> Dict:
        ...
        audit_results["overall_compliance"] = (
            round(compliant_count / total_requirements * 100, 2)
    ...
    def detect_drift(self, current_audit: Dict, baseline_audit: Dict) -> Dict:
        ...
        return {
            ...
            "severity": (
                "عالي" if abs(drift) > 15 else "متوسط" if abs(drift) > 5 else "منخفض"
            ),
    ...
    def generate_compliance_report(self, audit_id: str = None) -> Dict:
        ...
        return {
            ...
            "average_compliance": round(
                sum(a["overall_compliance"] for a in audits_to_report)
                / len(audits_to_report),
                2,
            ),
            ...
            "ready_for_external_review": all(
                a["overall_compliance"] >= 85 for a in audits_to_report
            ),
        }
    ...
    def _load_iso45001_requirements(self) -> List[Dict]:
        ...
        return [
            ...
            {
                "id": "6.1",
                "name": "الإجراءات لمعالجة المخاطر والفرص",
                "category": "planning",
            },
            {"id": "7.2", "name": "الكفاءة", "category": "support"},
            {
                "id": "8.1",
                "name": "التخطيط والسيطرة التشغيلية",
                "category": "operation",
            },
            {
                "id": "9.1",
                "name": "المراقبة والقياس والتحليل والتقييم",
                "category": "evaluation",
            },
            {
                "id": "10.1",
                "name": "عدم المطابقة والإجراء التصحيحي",
    ...
    def _load_osha_requirements(self) -> List[Dict]:
        ...
        return [
            ...
            {
                "id": "1910.147",
                "name": "التحكم في الطاقة الخطرة",
                "category": "lockout_tagout",
            },
            {
                "id": "1926.501",
                "name": "حماية من السقوط",
    ...
    def _get_drift_recommendations(self, drift: float) -> List[str]:
        ...
        if drift < -10:
            return ["تدقيق فوري مطلوب", "مراجعة شاملة للإجراءات", "تدريب طارئ"]
        elif drift < 0:
    ...
    def _identify_common_issues(self, audits: List[Dict]) -> List[str]:
        ...
        return ["نقص في التدريب", "معدات غير محدثة", "توثيق ناقص"]
