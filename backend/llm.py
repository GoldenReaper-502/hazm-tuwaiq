...
def _build_system_prompt(detection_context: Optional[Dict]) -> str:
    ...
    if detection_context:
        ...
        if objs:
            ...
            for o in objs[:5]:
                parts.append(
                    f"  • {o.get('class', 'unknown')} (confidence: {o.get('confidence', 0):.2f})"
                )
...
def generate_llm_response(
    user_message: str, detection_context: Optional[Dict], session_history: List[Dict]
) -> Dict:
        ...
        try:
            ...
            for item in (session_history or [])[-6:]:
                if item.get("user_message"):
                    chat_history.append(
                        {"role": "user", "parts": [item["user_message"]]}
                    )
                if item.get("assistant_response"):
                    chat_history.append(
                        {"role": "model", "parts": [item["assistant_response"]]}
                    )
    ...
    if OPENAI_KEY:
        try:
            ...
            response = client.chat.completions.create(
                model=OPENAI_MODEL, messages=messages, max_tokens=600, temperature=0.3
            )
