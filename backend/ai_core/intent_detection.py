...
class IntentDetector:
    ...
    def detect_intent(
        ...
        dangerous_intents = self._detect_dangerous_intents(
            movement_analysis, predicted_path, person_pose, context
        )

        # Calculate overall risk
        risk_assessment = self._assess_risk(
            movement_analysis, collision_risks, dangerous_intents
        )
    ...
    def _update_history(
        self, position: Tuple[float, float], gaze: Optional[Tuple[float, float]]
    ):
    ...
    def _detect_movement_pattern(self) -> str:
        ...
        direction_variance = np.var([d[0] for d in directions]) + np.var(
    ...
    def _check_collision_risks(
        ...
        for point in trajectory:
            for zone in self.danger_zones:
                ...
                distance = np.sqrt(
                    (point[0] - zone["x"]) ** 2 + (point[1] - zone["y"]) ** 2
                )
    ...
    def _detect_dangerous_intents(
        ...
        if trajectory:
            ...
            for zone in self.danger_zones:
                distance = np.sqrt(
                    (last_point[0] - zone["x"]) ** 2 + (last_point[1] - zone["y"]) ** 2
                )
    ...
    def _assess_risk(
        self, movement: Dict[str, Any], collisions: List[Dict], intents: List[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": message,
            "unhappened_accidents_prevented": len(collisions)
            + len([i for i in intents if i["risk"] == "HIGH"]),
        }
